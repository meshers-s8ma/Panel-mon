...
def delete_role(role_id):
    """Обрабатывает удаление роли с проверками."""
    role = db.get_or_404(Role, role_id)
...
def edit_user(user_id):
    """Обрабатывает редактирование существующего пользователя."""
...
def delete_user(user_id):
    """Обрабатывает удаление пользователя с проверками безопасности."""
    user_to_delete = db.get_or_404(User, user_id)
    
    # --- НАЧАЛО ИСПРАВЛЕНИЯ: Более надежная логика проверок ---
    # Сначала проверяем, не является ли удаляемый пользователь последним администратором.
    # Эта проверка имеет наивысший приоритет.
    ...
    # Затем проверяем, не пытается ли пользователь удалить сам себя.
