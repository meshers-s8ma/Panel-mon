...
from wtforms import (StringField, PasswordField, BooleanField, SubmitField,
                     SelectMultipleField, SelectField, IntegerField, TextAreaField, HiddenField)
...
class EditPartForm(FlaskForm):
    """Форма для редактирования данных детали."""
...
class AddChildPartForm(FlaskForm):
    """Форма для добавления дочернего узла к детали."""
...
class GenerateFromCloudForm(FlaskForm):
    """Форма для генерации отчета из облачного файла."""
...
class ConfirmStageQuantityForm(FlaskForm):
    ...
    # НОВОЕ ПОЛЕ для определения действия (выполнено, брак, переделка)
